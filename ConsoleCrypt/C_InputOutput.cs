    ...
    class C_InputOutputFile:I_InputOutput
    {
        ...
        public I_INPUTOUTPUTMESSAGE CryptFile(string key)
        {
            try
            {
                ...
                string content;
                using (StreamReader srDecrypt = new StreamReader(appSettings.DirDecryptFile, Encoding.UTF8))
                {
                    using (StreamWriter swCrypt = new StreamWriter(appSettings.DirCryptFile, false, Encoding.UTF8))
                    {
                        while (true)
                        {
        ...
        public I_INPUTOUTPUTMESSAGE DecryptFile(string key)
        {
            try
            {
                ...
                if (String.IsNullOrWhiteSpace(key) || String.IsNullOrEmpty(key))
                    return I_INPUTOUTPUTMESSAGE.KeyIsNull;
                string content;
                using (StreamReader srCrypt = new StreamReader(appSettings.DirCryptFile, Encoding.UTF8))
                {
                    using (StreamWriter swDecrypt = new StreamWriter(appSettings.DirCryptFile, false, Encoding.UTF8))
                    {
                        while (true)
                        {
